...
pygame.display.set_caption("Yılan Oyunu - (bomba elması güncellemesi!)")
...
class Oyun:
    def __init__(self, hiz=10, sure_limit=30):
        ...
        self.mesaj = ""
        self.mesaj_zamani = 0
        self.ekran_salla = False
        self.son_patlama_zamani = 0
        ...
        while True:
            ...
            if yeni_bas == self.elma.konum:
                if self.elma.tur == "kirmizi":
                    ...
                    self.yilan.kısalt(0)
                elif self.elma.tur == "altin":
                    self.skor += 3
                    self.mesaj = "ALTIN ELMAYI KAPTIN !"
                    self.yilan.kısalt(-2)
                elif self.elma.tur == "bomba":
                    self.skor = max(0, self.skor - 2)
                    self.yilan.kısalt(2)
                    self.mesaj = "BOOM! Dikkat et!"
                    self.ekran_salla = True
                    self.son_patlama_zamani = time.time()

                self.mesaj_zamani = time.time()
                if self.skor % 5 == 0 and self.hiz < 30 and self.elma.tur != "bomba":
                    self.hiz += 2
                self.elma = Elma(self.yilan.beden, self.engeller)
            else:
                self.yilan.kısalt()
    ...
    def ekrani_guncelle(self, gecen):
        if self.ekran_salla and time.time() - self.son_patlama_zamani < 0.2:
            ekran.fill(kirmizi)
        else:
...
class Elma:
    def _init_(self, yilan, engeller):
        self.tur, self.konum = self.yeni_elma_uret(yilan, engeller)
...
class Oyun:
    def _init_(self, hiz=10, sure_limit=30):
        self.yilan = Yilan()
        self.engeller = [
            (200, 100), (200, 120), (200, 140),
            (400, 200), (420, 200), (440, 200),
